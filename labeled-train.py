...
def get_data():
    ...
    i = 0
    while i < labeled_num:
        img = Image.open(data_dir + '/test' + '/' + str(i) + '.png')
        img = np.array(img, dtype=np.float32) / 255
        img = img[np.newaxis, np.newaxis, :, :]

        mask = io.loadmat(data_dir + '/test_masks' + '/' + str(i) + '.mat')['img']
...
def train(model, imgs, masks, optimizer, epoch, step_counter):
    ...
    hist = np.zeros((3), dtype=np.float)
    for jj in range(len(imgs)):
        h, bins = np.histogram(np.array(masks,np.uint8), list(range(3 + 1)))
        hist += h

    hist = hist / (max(hist))
    criterion_weight = 1 / np.log(1.02 + hist)
    # criterion_weight[0] = 0
    aa = nn.CrossEntropyLoss(Variable(torch.from_numpy(criterion_weight).float().cuda()))
    # criterionL = nn.NLLLoss(Variable(torch.from_numpy(criterion_weight).float().cuda()))
    model.train()
    j = 20
    for i in range(len(imgs)):
        ...
        maskk = torch.from_numpy(np.array(mask, dtype=np.long))
        ...
        c, mean, acc = compared(masks_preds, mask, i)
        # c, acc = compared1(masks_preds, mask, i)

        loss = aa(model_out, target_var)
        # loss = ccc(model_out, aabb)
        # loss = loss1 + 2*loss2
        print('i={}, dice={:.3f} , acc={:.3f} of {}, loss={:.3f}'.format(i, mean, acc, c, loss))
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        step_counter += 1
